// Main entry point for the Socialite web client
...
const fetchPosts = async () => {
  try {
    ...
    if (!unpublishedResponse.ok) throw new Error('Failed to fetch unpublished posts');
    state.unpublishedPosts = await unpublishedResponse.json();
    
    // Fetch published posts
    const publishedResponse = await fetch('/api/posts?published=true');
    if (!publishedResponse.ok) throw new Error('Failed to fetch published posts');
...
const renderApp = () => {
  ...
  app.innerHTML = `
    <header class="header">
      <div class="header-logo">Socialite</div>
      <nav class="header-nav">
        <button class="btn ${state.currentView === 'unpublished' ? 'btn-primary' : ''}" 
          data-view="unpublished">Unpublished</button>
        <button class="btn ${state.currentView === 'published' ? 'btn-primary' : ''}" 
          data-view="published">Published</button>
        <button class="btn ${state.currentView === 'calendar' ? 'btn-primary' : ''}" 
    ...
    <footer class="footer">
      <p>Socialite - AI-powered social media scheduler</p>
    </footer>
  `;
  ...
  document.querySelector('[data-action="create-post"]').addEventListener('click', () => {
...
const renderUnpublishedPosts = () => {
  if (state.unpublishedPosts.length === 0) {
    ...
    // Set up event listener for create post button
    mainContent.querySelector('[data-action="create-post"]').addEventListener('click', () => {
      state.currentView = 'editor';
      state.currentPost = null;
      renderApp();
    });
  ...
  mainContent.innerHTML = `
    <div class="section">
      ...
      <div class="post-list">
        ${state.unpublishedPosts.map(post => `
          <div class="card post-card" data-post-id="${post.id}">
            ...
            <div class="post-card-footer">
              ...
              <div class="d-flex gap-sm">
                <button class="btn btn-sm" data-action="edit-post" data-post-id="${post.id}">Edit</button>
                <button class="btn btn-sm btn-action" data-action="publish-post" data-post-id="${post.id}">Publish</button>
              </div>
            </div>
          </div>
        `).join('')}
  ...
  // Edit post buttons
  mainContent.querySelectorAll('[data-action="edit-post"]').forEach(button => {
    button.addEventListener('click', () => {
  ...
  mainContent.querySelectorAll('[data-action="publish-post"]').forEach(button => {
...
const renderPublishedPosts = () => {
  if (state.publishedPosts.length === 0) {
    ...
    mainContent.querySelector('[data-view="unpublished"]').addEventListener('click', () => {
...
const renderPostEditor = () => {
  ...
  const selectedPlatforms = post.platforms ? post.platforms.split(',').map(p => p.trim()) : [];
  
  mainContent.innerHTML = `
    <div class="card post-editor">
      <div class="post-editor-header">
        <h2 class="post-editor-title">${isEditing ? 'Edit Post' : 'Create New Post'}</h2>
      </div>
  ...
  document.getElementById('post-form').addEventListener('submit', async (event) => {
    ...
    const platformElements = document.querySelectorAll('input[name="platforms"]:checked');
    const platforms = Array.from(platformElements).map(el => el.value).join(',');
    ...
    try {
      if (isEditing) {
        ...
        const unpubIndex = state.unpublishedPosts.findIndex(p => p.id === post.id);
        if (unpubIndex !== -1) state.unpublishedPosts[unpubIndex] = updatedPost;
      } else {
        // Create new post
          ...
          return `
            <div class="calendar-day ${isToday ? 'calendar-day-today' : ''}" data-day="${day}">
              ...
              <div class="calendar-day-content">
                ${dayPosts.map(post => `
                  <div class="calendar-day-post" title="${post.title || 'Untitled'}" data-post-id="${post.id}">
                    ${post.title || 'Untitled'}
                  </div>
                `).join('')}
                ${dayPosts.length === 0 ? '' : `
                  <div class="calendar-day-count">${dayPosts.length} post${dayPosts.length > 1 ? 's' : ''}</div>
                `}
              </div>
            </div>
          `;
...
const publishPost = async (postId) => {
  try {
    const button = document.querySelector(`[data-action="publish-post"][data-post-id="${postId}"]`);
