...
if (blueskyHandle && blueskyPassword) {
  ...
  if (!config.defaultPlatforms || !config.defaultPlatforms.includes('bluesky')) {
  ...
  if (process.stdout.isTTY) {
    console.log('Please run the following command to set up Bluesky credentials:');
    console.log('socialite init');
  }
}

// Save the updated config
const configPath = path.join(os.homedir(), '.socialite', 'config.json');
