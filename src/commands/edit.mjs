...
export const editPost = async (argv) => {
  try {
      ...
      const { selectedIndex } = await inquirer.prompt([
        {
          ...
          choices: posts.map((post, index) => ({
            name: `[${index + 1}] ${post.title || 'No title'} - ${post.content.substring(0, 40)}...`,
            value: index + 1
          }))
        }
      ]);
    ...
    if (postIndex < 1 || postIndex > posts.length) {
      console.error(chalk.red(`Invalid post index. Must be between 1 and ${posts.length}.`));
      process.exit(1);
    }
    ...
    const responses = await inquirer.prompt([
      ...
      },
      {
        ...
        choices: [
          ...
          { name: 'Bluesky', value: 'Bluesky', checked: post.platforms && post.platforms.includes('Bluesky') },
    ...
    if (success) {
      ...
      console.log(chalk.gray('Run'), chalk.cyan('socialite unpublished'), chalk.gray('to see your updated post.'));
      console.log(chalk.gray('Run'), chalk.cyan('socialite publish'), chalk.gray('to publish eligible posts.'));
    } else {
